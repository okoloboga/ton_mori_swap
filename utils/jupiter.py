...
class JupiterAPI:
    ...
    async def initiate_swap(self, amount: float, meme_coin: str, solana_wallet: str, commission: float, fee_wallet: str) -> str:
        config = get_config(MemeCoinConfig, "meme_coin")
        ...
        if amount < config.min_swap_amount:
            logger.error(f"Swap amount too low: {amount} USDC, minimum is {config.min_swap_amount} USDC")
            raise ValueError(f"Minimum swap amount is {config.min_swap_amount} USDC")
